    ...
    fetchCareerById(id)
      .then((data) => {
        ...
        if (!cancelled) {
          ...
          setRelatedCareers(data.relatedCareers);
        }
      })
      .catch(() => {
    ...
    <div className="min-h-screen bg-surface-inset pb-12">
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {relatedCareers.length > 0 && (
          <div className="bg-surface-raised border border-border p-8 rounded-xl shadow-sm mt-8">
            <h2 className="text-xl font-bold text-ink mb-6">Related Careers</h2>

            <div className="grid gap-4 sm:grid-cols-2">
              {relatedCareers.map((related) => (
                <Link
                  key={related._id}
                  to={`/careers/${related._id}`}
