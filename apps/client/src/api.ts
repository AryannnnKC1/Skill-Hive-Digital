...
const rawBaseUrl =
  import.meta.env.VITE_API_URL || 'https://skill-hive-digital.onrender.com/api';
...
export async function fetchSavedCareers() {
  const response = await api.get<{ savedCareers: SavedCareerRecord[] }>(
    '/careers/saved'
  );
...
export async function saveCareer(careerId: string) {
  const response = await api.post<{ saved: SavedCareerRecord }>(
    '/careers/saved',
    {
      careerId,
    }
...
export async function fetchActiveAssessment() {
  const response = await api.get<{ assessment: Assessment }>(
    '/assessments/active'
  );
