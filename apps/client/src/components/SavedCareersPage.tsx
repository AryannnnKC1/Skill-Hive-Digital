...
export function SavedCareersPage({ entries, loading, error, onRemove, onToggleSave, isSaved }: SavedCareersPageProps) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="bg-surface-raised border border-border rounded-xl p-6 shadow-sm">
              <div className="mb-4 h-4 w-24 rounded bg-surface-inset animate-pulse" />
              <div className="mb-4 h-6 w-40 rounded bg-surface-inset animate-pulse" />
              <div className="h-20 rounded bg-surface-inset animate-pulse" />
            </div>
          ))}
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="bg-surface-raised border border-border rounded-xl p-6 flex items-start gap-3">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-ink-muted w-6 h-6">
  ...
  if (!entries.length) {
    return (
      <div className="bg-surface-raised border border-border rounded-xl p-12 text-center">
        <div className="w-16 h-16 rounded-full bg-surface-inset flex items-center justify-center mx-auto mb-6">
        ...
        <h2 className="mb-3 text-2xl font-bold text-ink">No saved careers yet</h2>
        <p className="mb-8 text-ink-muted max-w-md mx-auto">
          Bookmark careers you like from the catalog, and they'll appear here for quick access later.
        </p>
        <Link to="/careers" className="bg-cta hover:bg-cta-hover text-white font-medium rounded-lg px-6 py-2.5 transition-colors inline-flex items-center justify-center cursor-pointer">
          Browse careers
        </Link>
      </div>
    )
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <h2 className="text-3xl font-bold text-ink">Your shortlist</h2>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        {entries.map(({ career }) => (
          <article key={career._id} className="bg-surface-raised border border-border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col">
            ...
            <p className="mb-6 text-sm leading-relaxed text-ink-muted line-clamp-3 flex-grow">
              {career.description}
            </p>
            ...
            <div className="mt-auto pt-4 border-t border-border flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-4">
                <button type="button" onClick={() => void onRemove(career._id)} className="font-medium text-ink-subtle hover:text-ink transition-colors cursor-pointer">
                  Remove
                </button>
